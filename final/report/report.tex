...
\abstract{This is our final report for the robotics course 2011 of the
University of Amsterdam. Our project is to create a communication channel
between a Jobot and Sun SPOT, and communicate wirelessly between multiple Sun
SPOTs. Communication also includes bit banging, error checking and programming
the devices to handle the transmission properly, emitting low-level signals to
command the servo's using our C driver and, last but not least, we composed a
toolchain to program our C driver on the Jobot.}
