...
\frame{\frametitle{Topolgical Maps}
 ...
 \begin{itemize}
 	\item Metric representations cost too much memory to maintain in the long run.
 	\item Easy for us to understand aswell.
 \end{itemize}
...
\frame{\frametitle{The Graph}
	...
	\begin{itemize}
		...
		\item Landmarks can be artificial or natural.
	 	\item The graph can be extended by enumerating the edges incident to the node entered. 
	\end{itemize}	
}
...
\frame{\frametitle{Marker Based Exploration}
 ...
 \begin{itemize}
 	...
 	\item Using marks to explore has a $O(N^{3})$.
 \end{itemize} 
} 
...
\frame{\frametitle{Marker Based Exploration}
	...
	Edge you traveled along is 0 and enumerate clockwise. This enumeration is local cause it depends on the edge the robot moved over.
...
\frame{\frametitle{Marker Based Exploration}
	...
	\begin{itemize}
		\item Move along an edge.		
		\item Can do 3 different to a marker [pickup,putdown,null].
...
\frame{\frametitle{}
	...
	\begin{itemize}
		\item First validate all known nodes. So all nodes in graph S. Make sure there aren't any doubles by looking for markers.
		\item	Explore new nodes. If there is no marker found at a certain node v add it the subgraph S, add the edge which was taken aswell and drop a marker.
...
\frame{\frametitle{Multiple Robots}
	...
	\begin{itemize}
		\item \textbf{Improved Robustness}: A multirobot system can, in principle, keep functioning even if one indiviudual robots dail completely. 
...
\frame{\frametitle{Map Fusion}
	...
	\begin{itemize}
		...
		\item Cross correlation is done by rotation and translating the given maps till you minimized the difference between them.
	\end{itemize}	
}
...
\frame{\frametitle{Exploration with Multiple robots}
	...
	\begin{itemize}
		...
		\ítem Plan rendezvous, to harmonize the information they got till then and make a single consistent representation of the environment they are in.
...
\frame{\frametitle{Conclusion}
	\begin{itemize}
		\item Topological Maps are low cost, but still give a decent representation of the environment.
