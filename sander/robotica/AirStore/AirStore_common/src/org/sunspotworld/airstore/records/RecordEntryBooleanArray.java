...
/**
 *  Class for a simple key-value pair in which the value is an array of booleans.
 *  The key (iherited) must be a string. 
 */
public class RecordEntryBooleanArray extends RecordEntryTypedArray {
   ...
   public void initializeValueFrom(DataInput s) {
    ...
    public void copyValueInto(RecordEntry re) {
        ((RecordEntryBooleanArray) re).setValue(value);
    }
    ...
    public boolean matchesEntry(RecordEntry re){
        ...
        if(! (re instanceof RecordEntryBooleanArray)){
            print("          = NO key match, type mismatch ");
            return false;
        }  //Key match, but value types don't -> false
        boolean[] ourArr = (boolean[]) getValue();
        boolean[] itsArr = (boolean[]) re.getValue();
        ...
        if(same){
            print("          = YES key match, type match, array length match, all booleans == in order");
        } else{
            print("          = NO key match, type match, array length match, but 1 or more booleans not == in order");
        }
        print("          =========");
        return same;
    }
    ...
    public void parseElementAddToVector(String token, Vector valVector) throws CannotParseException {
        Boolean val;
        System.out.println("Was asked to parse " + token);
        if (token.equalsIgnoreCase("true")) {
            val = Boolean.TRUE;
        } else if (token.equalsIgnoreCase("false")) {
            val = Boolean.FALSE;
        } else {
            throw new CannotParseException(token + " does not parse as true or false");
        }
        System.out.println("Was asked to parse " + token);
    ...
    public void moveElementsToValue(Vector valueVector) {
        value = new boolean[valueVector.size()];
        for (int i = 0; i < value.length; i++) {
            value[i] = ((Boolean) valueVector.elementAt(i)).booleanValue();
        }
    }
}
