...
/**
 *  Class for a simple key-value pair in which the value is an array of longs.
 *  The key (iherited) must be a string. 
 */
public class RecordEntryLongArray extends RecordEntryTypedArray {
    ...
    public void copyValueInto(RecordEntry re) {
        ...
        ((RecordEntryLongArray) re).setValue(newArray);
    }
    ...
    public boolean matchesEntry(RecordEntry re){
        ...
        if(! (re instanceof RecordEntryLongArray)){
            print("          = NO key match, type mismatch ");
            return false;
        }  //Key match, but value types don't -> false
        long[] ourArr = (long[]) getValue();
        long[] itsArr = (long[]) re.getValue();
        ...
        if(same){
            print("          = YES key match, type match, array length match, all longs == in order");
        } else{
            print("          = NO key match, type match, array length match, but 1 or more longs not == in order");
        }
        print("          =========");
        return same;
    }
    ...
    public void parseElementAddToVector(String token, Vector valVector) throws CannotParseException {
        try {
            ...
            valVector.addElement(new Long(val));
    ...
    public void moveElementsToValue(Vector valueVector) {
        value = new long[valueVector.size()];
        for (int i = 0; i < value.length; i++) {
            value[i] = ((Long) valueVector.elementAt(i)).longValue();
        }
    }
}
